...
class AddNewCalssMove extends StatefulWidget {
  const AddNewCalssMove({Key? key}) : super(key: key);

  @override
  State<AddNewCalssMove> createState() => _AddNewCalssMoveState();
}

class _AddNewCalssMoveState extends State<AddNewCalssMove>with Helpers {
  ...
  Future<void>addCategory()async{
    bool status = await FbStoreController().addCategory(expense: category);
    if(status)
    {
...
CategoryExpense get category {

    CategoryExpense categoryExpense = CategoryExpense();
    categoryExpense.name = _textAddClassMove.text;
